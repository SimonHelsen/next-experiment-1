import * as React from 'react';
import { ContextualSaveBarProps as Props } from '../../../ContextualSaveBar';
import { WithAppProviderProps } from '../../../AppProvider';
export declare type CombinedProps = Props & WithAppProviderProps;
export interface State {
    discardConfirmationModalVisible: boolean;
}
declare class ContextualSaveBar extends React.PureComponent<CombinedProps, State> {
    state: State;
    render(): JSX.Element;
    private toggleDiscardConfirmationModal;
}
declare const _default: React.ComponentClass<Props, React.ComponentState> & typeof ContextualSaveBar;
export default _default;
