...
export class ResourceList extends React.Component {
    ...
    render() {
        const { items, promotedBulkActions, bulkActions, filterControl, loading, showHeader = false, sortOptions, sortValue, onSortChange, polaris: { intl }, } = this.props;
        ...
        const sortingSelectMarkup = sortOptions && sortOptions.length > 0 ? (<div className={styles.SortWrapper}>
          {sortingLabelMarkup}
        ...
        const headerMarkup = (showHeader || needsHeader) &&
            listNode && (<div className={styles.HeaderOuterWrapper}>
          <Sticky boundingElement={listNode}>
            {(isSticky) => {
            const headerClassName = classNames(styles.HeaderWrapper, sortOptions &&
                sortOptions.length > 0 &&
                styles['HeaderWrapper-hasSort'], this.selectable && styles['HeaderWrapper-hasSelect'], loading && styles['HeaderWrapper-disabled'], this.selectable &&
                selectMode &&
        ...
        const emptyStateMarkup = filterControl && !itemsExist && !loading ? (<div className={styles.EmptySearchResultWrapper}>
        ...
        const listMarkup = itemsExist ? (<ul className={resourceListClassName} ref={this.listRef} aria-live="polite" aria-busy={loading}>
        {loadingOverlay}
    ...
    renderItem(item, index) {
        ...
        return (<li key={id} className={styles.ItemWrapper} tabIndex={tabIndex}>
        {renderItem(item, id)}
      </li>);
    }
